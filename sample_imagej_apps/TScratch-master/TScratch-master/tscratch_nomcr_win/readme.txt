    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_TScratch.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * TScratch.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * TScratch.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * TScratch (application created by MATLAB Compiler)
        * run_TScratch.sh (shell script that can be run to 
    ...
    All Platforms:
        * TScratch.dll (shared library, file extension varies by 
          platform)
        * TScratch.h (library header file)
        * TScratch.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * TScratch.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * TScratch.xml (documentation files)
        * TScratch.pdb (program database file - if DEBUG option 
          selected)
        * TScratch.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * TScratch.ctf (Component Technology File archive, contains 
          compiled M-code)
        * TScratch_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * TScratch.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * TScratch.ctf (Component Technology File archive, contains 
          compiled M-code)
        * TScratch.xla (Optionally created and placed in
          C:\Documents and Settings\Tobias\My Documents\MATLAB\ScratchAssay\TScratch_win\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
