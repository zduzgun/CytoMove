...
These tools also leave practical gaps that are visible from the standpoint of everyday assay analysis. TScratch and PyScratch provide dedicated interfaces, but they still require local software environments and are less naturally connected to modern browser-based sharing, review, and export workflows. WHST benefits from the maturity and familiarity of ImageJ/Fiji, but it depends on plugin-based use, manual parameter tracking, and a separate record-keeping layer if the user wants a fully auditable export package. CSMA adds valuable attention to cells inside the wound region, but it remains centred on an external analysis environment rather than a browser-local workflow in which image loading, segmentation review, quality-control labels, metadata capture, plots, overlays, and spreadsheet export are kept together. These are not weaknesses of the prior tools so much as open workflow needs: lower installation friction, clearer provenance, easier group review, and more explicit linkage between the exported number and the segmentation image that produced it.
...
Cytomove uses an explainable, dependency-free image-processing pipeline built from general microscopy image-analysis operations: luminance conversion, field-of-view masking, contrast normalisation, local texture measurement, thresholding, connected-component filtering, hole filling, and geometric width estimation. The pipeline is implemented independently in JavaScript using typed arrays, without ImageJ/Fiji, WHST, OpenCV, or other external image-processing backends. WHST is used in this manuscript as a quantitative comparator because it reports a closely related scratch-assay measurement vocabulary; the WHST plugin or code is not invoked by Cytomove. The current Cytomove algorithm version is recorded in every export. The pipeline proceeds through the following stages:
...
4. **Local variance map.** A local variance is computed for every pixel within a user-selected radius. The computation uses an integral image (summed-area table) so that the per-pixel variance is obtained in constant time regardless of radius. This provides an efficient browser implementation of a standard local-texture descriptor: confluent cell regions have high local texture variance, whereas the cell-free wound corridor is comparatively smooth.
...
7. **Hole filling.** Holes inside the wound region are filled using a standard border flood-fill and inversion procedure, so that small cell clusters inside the gap do not punch holes in the wound mask.
...
The validation design was structured around four local scratch-assay tool papers (TScratch, WHST, PyScratch, and CSMA), which were used to define the comparison language and the minimum evidence standard for this work (Table 1). WHST was selected as the primary quantitative benchmark, not as an algorithmic template, because its ImageJ/Fiji output vocabulary overlaps with Cytomove's exported endpoints: wound area, wound area fraction, average wound width, and width variability (Suarez-Arnedo et al., 2020). TScratch was treated as a design comparator for reviewable, correctable segmentation (Gebäck et al., 2009). PyScratch was treated as a usability and time-course comparator because it emphasises batch processing and non-programmer interaction (Garcia-Fossa et al., 2020). CSMA was treated as an emerging comparator for wound-internal cell handling, time-course outputs, and visual audit panels (Pham et al., 2025).

TScratch and PyScratch were not used as primary numerical benchmarks in this preliminary validation. TScratch was used mainly to motivate reviewable and correctable segmentation because its historical contribution is strongest at the workflow-design level, and its MATLAB/standalone environment is less convenient for a small reproducible benchmark package. PyScratch was likewise treated as a usability comparator rather than a metric benchmark because the present study focuses on per-frame agreement for area and width against a widely used ImageJ/Fiji output vocabulary. Adding multi-tool benchmarking, including direct TScratch and PyScratch reruns on the same image sets, is therefore reserved for the larger technical validation phase.
...
The same CSMA sample sequence also allowed a three-method area comparison because native CSMA area outputs, WHST outputs, and Cytomove measurements were available for the matched frames (Figure 5). All three methods followed the same decreasing wound-area trajectory, with high trend correlation between CSMA and WHST (r = 0.9970) and between CSMA and Cytomove (r = 0.9951). The frame-level pattern is nevertheless informative: native CSMA retained larger residual wound-area values in later frames than WHST and Cytomove, giving a CSMA-versus-WHST area MAPE of 20.1% and a Cytomove-versus-CSMA MAPE of 17.6%. This divergence is presented as method-context evidence rather than as a failure case, because CSMA explicitly targets wound-internal cell handling and produces a different native area output. The result supports using CSMA as a visual and trend comparator while keeping WHST as the primary quantitative benchmark for the present manuscript.

To make this comparison inspectable rather than purely numerical, the CSMA three-method trend and the frame-level visual audit were combined into a single main figure (Figure 5). The lower audit panel shows the raw image, the CSMA area output, the WHST output, and the Cytomove overlay for each of the 11 frames. This layout confirms that the high WHST-Cytomove numerical agreement corresponds to visually concordant wound corridors, and it also makes the later CSMA native-output differences visible in image space, in keeping with the comparator literature's emphasis on pairing numbers with image evidence (Figure 3; Figure 4; Figure 5).
...
Across the five sets, area agreement with WHST was strong in the clean and phase-contrast time courses and degraded predictably in the hardest phone captures, while the diagnostic statistics behaved consistently with the near-closure and fragmentation effects described above (Table 3; Supplementary Figure S1; Supplementary Table S1). A pooled Bland–Altman comparison on the absolute area scale (Figure 3B) showed a median Cytomove-minus-WHST difference of essentially zero (−0.002 Mpx), with 2.5–97.5th percentile limits of agreement of −0.134 to +0.169 Mpx. An absolute, non-parametric summary is reported here rather than a mean percentage bias because a percentage difference is unstable for near-closure frames in which the residual wound spans only a few thousand pixels, and because the 31 pooled measurements include two time-course sequences whose frames are repeated observations of the same wound rather than independent samples; the limits of agreement should therefore be read as descriptive of the observed spread rather than as formal confidence bounds. Area was the more stable and comparable metric throughout; width tracked area but was more sensitive in fragmented and near-closed frames. This supports the area-first, width-as-QC interpretation adopted in this work.
...
This study did not involve human participants, identifiable human data, or animal experiments, and therefore required no ethical approval. The MCF-7 phase-contrast frames were obtained from a publicly available dataset released under the Creative Commons Attribution 4.0 International licence (Iheme et al., 2024), and the smartphone-through-eyepiece brightfield images were acquired by the author from routine in vitro cell cultures. All other images used as analysis inputs were derived from publicly available sources as described in the Image and Data Provenance section.
...
**Figure 1. Cytomove application workspace for browser-local scratch assay quantification.** The browser-local interface keeps (A) adjustable segmentation parameters, (B) overlay-based wound review, (C) quality-control and metric summaries, and (D) frame-level group review in a single workflow before export.
...
**Figure 3. WHST versus Cytomove wound-area comparison.** (A) Paired WHST and Cytomove wound-area values are plotted against the identity line, with a distinct marker per dataset. (B) Bland–Altman view on the absolute area scale, showing the per-frame Cytomove-minus-WHST difference against the mean wound area; the solid line marks the median difference (−0.002 Mpx) and the dashed lines the 2.5–97.5th percentile limits of agreement (−0.134 to +0.169 Mpx). An absolute, non-parametric summary is used rather than a mean percentage bias because a percentage difference is unstable for near-closure frames with very small residual wounds, and because the pooled points include time-course frames that are not mutually independent.
...
**Figure 5. CSMA sample 11 three-method area trend and visual audit.** Native CSMA area output, WHST output, and Cytomove measurements are plotted across the matched 11-frame CSMA sample sequence (A-C), followed by a frame-level visual audit showing the raw image, CSMA area output, WHST output, and Cytomove overlay for each frame (D). The three methods follow the same decreasing wound-area trend, while the later frames show that native CSMA retains larger residual wound-area values than WHST and Cytomove. The larger CSMA-versus-WHST percentage differences in late frames reflect both method-specific residual-area behaviour and the small-denominator effect that occurs as the wound approaches closure. This panel places CSMA in the main result stream as a trend and visual-context comparator rather than as the primary quantitative benchmark.

**Figure 6. WHAD-MCF7 time-course behaviour and visual audit.** WHST and Cytomove wound-area (A) and wound-width (B) values are plotted across the 11-frame WHAD-MCF7 phase-contrast sequence from the WHAD/CAMAD Zenodo dataset (Iheme et al., 2024), followed by a frame-level visual audit showing the raw image, WHST output, and Cytomove overlay for each frame (C). Cytomove follows the time-course trend with close area and width agreement, while late near-closure frames are flagged as review-sensitive.
